...
async function handleEditForm(event) {
  try {
    ...
    */
    ...
    const formInputs = form[0].getElementsByTagName("input")
    const keys = ["title", "author", "year", "quantity"]
    const book = {}
    for (let i = 0; i < formInputs.length; i++) {
      const el = formInputs[i]
      if (el.type !== "submit") {
        book[keys[i]] = el.value
      }
    }
...
navLinks.forEach((navLink) => {
  // TODO: answer here
  navLink.onclick = handleClickAddNav
});
